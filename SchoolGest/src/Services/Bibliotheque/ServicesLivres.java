...
public class ServicesLivres implements IServices<Livre>{
    ...
    public boolean ajouter(Livre l) throws SQLException {
        PreparedStatement pre = con.prepareStatement("insert INTO `edutech`.`livre` (`titre`, `auteur`, `editeur`, `categorie`, `datesortie`, `taille`) VALUES (?, ?, ?, ?, ?, ?);");
    ...
    public boolean update(Livre l) throws SQLException {
        PreparedStatement pre = con.prepareStatement("update `edutech`.`livre` set `titre` = ?, `auteur` = ?, `editeur` = ?, `categorie` = ?, `datesortie` = ?, `taille` = ? where `id` = ?;");
        pre.setString(1, l.getTitre());
        pre.setString(2, l.getAuteur());
        pre.setString(3, l.getEditeur());
        pre.setString(4, l.getCategorie());
    ...
    public List<Livre> readAll() throws SQLException {
        ...
        while (rs.next()) {
            ...
            Livre l = new Livre(id, titre, auteur, editeur, categorie, datesortie, taille);
            listE.add(l);
        }
        return listE;
    }
